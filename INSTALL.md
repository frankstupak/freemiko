# Installing the MikoUnchained launcher

The MikoUnchained launcher (`app/mikounchained-debug.apk`) replaces the interim KISS home with a purpose-built
...
- Miko 3 already unlocked (`unlock/mikounchained-unlock.sh` completed) and booting to Android.
...
Leave it disabled — MikoUnchained's neuter replaces the only behaviour it enforced. Re-enable with
`adb shell pm enable com.example.root.serviceexam` if you ever want the stock watchdog back.

## 1. Install

```bash
adb install -r -g app/mikounchained-debug.apk
# if 'install' is blocked, push + install as root:
#   adb push app/mikounchained-debug.apk /data/local/tmp/
#   adb shell su 0 pm install -r -g /data/local/tmp/mikounchained-debug.apk
```

## 2. Grant the overlay app-op (so the nav bar draws with no on-screen toggle)

MikoUnchained self-grants this on first launch as root, but do it now so the bar appears immediately:

```bash
adb shell appops set com.mikounchained.launcher SYSTEM_ALERT_WINDOW allow
```

## 3. Make MikoUnchained the home, retire the Miko home + factory app

```bash
adb shell cmd package set-home-activity com.mikounchained.launcher/.HomeActivity
adb shell pm disable-user --user 0 com.miko.launcher_app
adb shell pm disable-user --user 0 com.miko.mikoplus
```

Verify MikoUnchained is HOME:

```bash
adb shell cmd package resolve-activity -c android.intent.category.HOME \
  -a android.intent.action.MAIN | grep packageName
# expect: packageName=com.mikounchained.launcher
```
...
```bash
adb shell am start -n com.mikounchained.launcher/.HomeActivity
```

## 5. Verify the watchdog neuter

```bash
adb shell su 0 sh -c 'cat /data/local/tmp/mikounchained/status'       # expect: OK
...
adb shell su 0 sh -c 'cat /data/local/tmp/mikounchained/neuter.log'   # human-readable trace
```
...
also a true reading (an app's in-process check would not be, which is why MikoUnchained relies on the
...
the mount clears on the next reboot and MikoUnchained's boot receiver re-applies it, with a self-healing
...
recents are injected as key events through root; home returns to the MikoUnchained drawer.
...
adb uninstall com.mikounchained.launcher
adb reboot   # clears the bind-mount neuter (never persisted to /system)
```

## Troubleshooting

- **Nav bar missing:** re-run step 2, then MikoUnchained → gear → "Restart nav bar".
- **Back/recents do nothing:** root/`su` not granting MikoUnchained. Home still works. Confirm
...
- **`status` shows `PENDING` and never `OK`:** the bind-mount isn't taking. Confirm `su` works
  (`adb shell su 0 id` returns uid 0) and that `/data/local/tmp/mikounchained/neuterd` exists and is
  executable.
